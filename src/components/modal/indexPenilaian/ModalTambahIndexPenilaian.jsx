...
import { createIndexPenilaian } from '../../../api/indexPenilaian';

export default function ModalTambahIndexPenilaian({ close, render }) {
  ...
  const handleChange = (target) => {
    ...
    if (target.name === 'grade') {
      helper.grade = target.value.toUpperCase();
      setDataInput(helper);
  ...
  const validation = () => {
    ...
    if (dataInput.batas_awal === undefined) {
      error.batas_awal = 'batas awal tidak boleh kosong';
      errStat.batas_awal = true;
      status = false;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (validation()) {
      try {
        const res = await createIndexPenilaian(dataInput);
        if (res) {
