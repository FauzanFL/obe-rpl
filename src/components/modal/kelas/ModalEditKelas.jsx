...
export default function ModalEditKelas({ close, render, data }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validation()) {
      try {
        const res = await updateKelas(dataInput, data.id);
        if (res) {
        ...
        <form action="" onSubmit={handleSubmit} className="overflow-auto">
          <div className="mb-2">
            ...
            <input
              ...
              placeholder="Masukkan kode kelas perancangan"
              defaultValue={data.kode_kelas}
              required
            />
